...
namespace Chess
{
    ...
    void UCIHandler::specificSetup(const YAML::Node& config)
    {
        ...
        m_perftTool = PerftTool(concreteEngine);
    }
    ...
    {
        State state{};
		this->m_engine->getInitialState(0, state);
		this->m_renderer->renderState(state);
        this->m_engine->changeStatePov(1, state);
        this->m_renderer->renderState(state);
        this->m_engine->changeStatePov(1, state);
        this->m_renderer->renderState(state);
