...
namespace Chess
{
    ...
    {        
        ...
        // 7 types de m�tadonn�es
        static constexpr uint32_t kNumMetaTypes = 6;


        // 32 slot de pi�ces physiques maximum sur le plateau en m�me temps
        static constexpr uint32_t kMaxElems = 32;

        // 9 slots de m�tadonn�es (Car les droits de roque prennent 4 slots, etc.)
    ...
    enum ChessEndReason : uint8_t
    {
        ...
        Checkmate = 1,
        Stalemate = 2,           // Pat
        Repetition = 3,          // Triple r�p�tition
        FiftyMoveRule = 4,       // R�gle des 50 coups
        InsufficientMaterial = 5,// Mat�riel insuffisant
        MaxPlyReached = 6,       // Limite de profondeur atteinte (pour les parties longues),
		Resigned = 7             // Résignation
    };
}