...
namespace Chess
{
	class ChessRenderer : public Core::IRenderer<ChessTypes>
	{
	public:
	...
	private:
		void renderFactState(const State& state) const;
	...
	public:
		...
		void renderState(const State& state) const override;
		void renderValidActions(const State& state, std::span<const Action> actionList) const override;
		void renderActionPlayed(const Action& action, uint32_t player) const override;
		void renderResult(const GameResult& result) const override;
	};
}